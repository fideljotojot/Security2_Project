...
BEGIN 
  INSERT INTO public.audit_logs(actor_id,action,entity_type,entity_id,details,ip_address) VALUES(auth.uid(),p_action,p_entity_type,p_entity_id,COALESCE(p_details,'{}'::jsonb),inet_client_addr()); 
...
RETURNS TABLE(log_id BIGINT,action TEXT,entity_type TEXT,entity_id TEXT,details JSONB,actor_username TEXT,actor_role TEXT,ip_address INET,created_at TIMESTAMPTZ) 
LANGUAGE plpgsql SECURITY DEFINER AS $$ 
BEGIN 
  IF NOT EXISTS(SELECT 1 FROM public.users WHERE id=auth.uid() AND role='superadmin') THEN 
    RAISE EXCEPTION 'Only superadmins can view audit logs'; 
  END IF; 
  RETURN QUERY SELECT l.id,l.action,l.entity_type,l.entity_id,l.details,u.username::TEXT,u.role::TEXT,l.ip_address,l.created_at FROM public.audit_logs l LEFT JOIN public.users u ON u.id=l.actor_id ORDER BY l.created_at DESC LIMIT LEAST(GREATEST(p_limit,1),100); 
END; $$;
