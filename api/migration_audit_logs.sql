...
CREATE OR REPLACE FUNCTION public.get_audit_logs(p_limit INTEGER DEFAULT 20) RETURNS TABLE(log_id BIGINT,action TEXT,entity_type TEXT,entity_id TEXT,details JSONB,actor_username TEXT,actor_role TEXT,created_at TIMESTAMPTZ) LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN IF NOT EXISTS(SELECT 1 FROM public.users WHERE id=auth.uid() AND role='superadmin') THEN RAISE EXCEPTION 'Only superadmins can view audit logs'; END IF; RETURN QUERY SELECT l.id,l.action,l.entity_type,l.entity_id,l.details,u.username::TEXT,u.role::TEXT,l.created_at FROM public.audit_logs l LEFT JOIN public.users u ON u.id=l.actor_id ORDER BY l.created_at DESC LIMIT LEAST(GREATEST(p_limit,1),100); END; $$;
