...
export default {
  ...
  computed: {
    ...
    filteredLogs() { const q = this.searchQuery.trim().toLowerCase(), now = Date.now(), ranges = { today: 86400000, week: 604800000, month: 2592000000 }; return this.logs.filter(log => (!q || [log.action, log.entity_type, log.entity_id, log.actor_username, log.actor_role, log.ip_address].join(' ').toLowerCase().includes(q)) && (this.selectedActor === 'all' || log.actor_username === this.selectedActor) && (this.selectedAction === 'all' || log.action === this.selectedAction) && (this.selectedRole === 'all' || log.actor_role === this.selectedRole) && (this.selectedTime === 'all' || now - new Date(log.created_at).getTime() <= ranges[this.selectedTime])); },
