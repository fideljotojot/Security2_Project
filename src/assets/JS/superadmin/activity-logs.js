...
export default {
  name: 'ActivityLogs', data: () => ({ logs: [], currentPage: 1, pageSize: 12, searchQuery: '', selectedActor: 'all', selectedAction: 'all', selectedTime: 'all', errorMessage: '' }),
  computed: {
    ...
    filteredLogs() { const q = this.searchQuery.trim().toLowerCase(), now = Date.now(), ranges = { today: 86400000, week: 604800000, month: 2592000000 }; return this.logs.filter(log => (!q || [log.action, log.entity_type, log.entity_id, log.actor_username].join(' ').toLowerCase().includes(q)) && (this.selectedActor === 'all' || log.actor_username === this.selectedActor) && (this.selectedAction === 'all' || log.action === this.selectedAction) && (this.selectedTime === 'all' || now - new Date(log.created_at).getTime() <= ranges[this.selectedTime])); },
  ...
  watch: { searchQuery() { this.currentPage = 1; }, selectedActor() { this.currentPage = 1; }, selectedAction() { this.currentPage = 1; }, selectedTime() { this.currentPage = 1; } },
