...
export default {
  data: () => ({ users: [], currentUserId: null, search: '', status: 'all', selectedRole: 'all', selectedPosition: 'all', sortOrder: '', currentPage: 1, pageSize: 10, message: '', notificationTitle: '', notificationType: 'success', notificationMessage: '', editing: null, viewing: false, editForm: {}, editStep: 'personal', deleteTarget: null, deleteReason: '', showPasswordModal: false, confirmationPassword: '', showConfirmationPassword: false, isConfirmingPassword: false }),
  ...
  async mounted() { const { data: { user } } = await supabase.auth.getUser(); this.currentUserId = user?.id || null; this.load(); },
  methods: {
    ...
    async setStatus(u, status) { if (!await confirmCurrentPassword('Enter your password to block or unblock this account:')) return; const { error } = await supabase.rpc('admin_update_user_status', { p_user_id: u.user_id, p_status: status }); if (error) this.message = error.message; else await this.load(); },
    ...
    async submitDelete() { if (!this.deleteReason.trim()) { this.notificationTitle = 'Reason Required'; this.notificationType = 'error'; this.notificationMessage = 'A deletion reason is required.'; this.deleteTarget = null; return; } const { error } = await supabase.rpc('create_delete_request', { p_user_id: this.deleteTarget.user_id, p_reason: this.deleteReason.trim() }); if (error) this.message = error.message; else { this.notificationTitle = 'Request Sent'; this.notificationType = 'success'; this.notificationMessage = 'Deletion request sent to the Superadmin.'; } this.deleteTarget = null; }
  }
};
