...
export default {
  ...
  methods: {
    ...
    async edit(u) { if (!u || u.role === 'superadmin' || this.userStatus(u) === 'blocked') return; const { data, error } = await supabase.rpc('get_full_user_for_admin_edit', { p_user_id: u.user_id }); if (error) this.message = error.message; else { this.editing = u; this.viewing = false; this.editForm = { ...data, password: '', repassword: '' }; this.editStep = 'personal'; } },
