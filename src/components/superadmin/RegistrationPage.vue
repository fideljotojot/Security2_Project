<template>
  <main class="container" style="overflow: auto ; display: flex; flex-direction: column">
    ...
    <div class="user-table">
      ...
      <div class="filter-container">
        <div class="searchbar"><input v-model="registrationSearch" type="search" placeholder="Search username or email"
            aria-label="Search pending registrations"><i class="fi fi-br-search" aria-hidden="true"></i></div>
        <div class="dropdown"><select v-model="registrationIdSort" aria-label="Sort by ID">
            <option value="">Sort by ID</option>
            <option value="ascending">Ascending order</option>
            <option value="descending">Descending order</option>
          </select><select v-model="selectedRegistrationTime" aria-label="Filter registrations by time">
        ...
        <tbody>
          <tr v-for="registration in paginatedRegistrations" :key="registration.user_id">
            ...
            <td class="registration-actions">
              ...
              <button type="button" class="block-action-btn reject-btn" title="Reject registration"
                aria-label="Reject registration" :disabled="isUpdating" @click="updateStatus(registration, 'blocked')">
      ...
      <div class="table-panel-footer"><span>Showing {{ pageStart(filteredRegistrations) }}-{{
        pageEnd(filteredRegistrations) }} of {{
    ...
    <div class="user-table" style="margin-top:1.5rem">
      <div class="header-section">
        ...
        <button type="button" @click="fetchDeleteRequests">Refresh</button>
      </div>
      <div class="filter-container">
        <div class="searchbar"><input v-model="deleteSearch" type="search" placeholder="Search username or email"
            aria-label="Search deletion requests"><i class="fi fi-br-search" aria-hidden="true"></i>
        </div>
        <div class="dropdown">
          <select v-model="deleteRole" aria-label="Filter deletion requests by role">
            <option value="all">All roles</option>
            <option value="user">User</option>
            <option value="admin">Admin</option>
            <option value="superadmin">Superadmin</option>
          </select>
          <select v-model="deleteIdSort" aria-label="Sort by ID">
            <option value="">Sort by ID</option>
            <option value="ascending">Ascending order</option>
            <option value="descending">Descending order</option>
          </select>
          <select v-model="selectedDeleteTime" aria-label="Filter deletion requests by time">
            <option value="all">All time</option>
            <option value="today">Today</option>
            <option value="week">Last 7 days</option>
            <option value="month">Last 30 days</option>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="request in paginatedDeleteRequests" :key="request.request_id">
            ...
            <td>{{ [request.first_name, request.middle_initial, request.last_name,
            request.suffix].filter(Boolean).join(' ') }}</td>
            ...
            <td>{{ request.sex }}</td>
            <td>
              {{ [request.purok, request.barangay, request.city, request.province,
              request.country].filter(Boolean).join(', ') }}
            </td>
            <td>{{ request.zip }}</td>
            ...
            <td class="registration-actions">
              ...
              <button type="button" class="block-action-btn reject-btn" title="Reject deletion"
                aria-label="Reject deletion" :disabled="isUpdating" @click="reviewDelete(request, false)">
                <i class="fi fi-br-cross-circle" aria-hidden="true"></i>
              </button>
            </td>
          </tr>
      ...
      <div class="table-panel-footer"><span>Showing {{ pageStart(filteredDeleteRequests) }}-{{
        pageEnd(filteredDeleteRequests) }} of {{
