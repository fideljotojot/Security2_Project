<template>
  <form action="" method="POST" @submit.prevent="login" style="padding: 1em 3em .8em;">
    ...
    <div class="warning-container" v-if="warnings.general && warnings.general.length">
      ...
      <ul>
        <li v-for="(msg, idx) in warnings.general" :key="idx" class="warning">{{ msg }}</li>
      </ul>
    </div>
    ...
    <a v-if="consecutiveError >= 2" @click="!lockoutActive && handleForgotPassword()"
      :class="{ disabled: lockoutActive }">Forgot Password</a>
  </form>
</template>
