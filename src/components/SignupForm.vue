  ...
  <form method="POST" @submit.prevent="register">
    ...
    <div class="form-content" v-if="step === 'personal'">
      ...
      <div class="registration-box">
        ...
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('suffix')">{{ getWarning('suffix') }}</span>
          <input type="text" id="suffix" name="suffix" placeholder="Jr, Sr, III, etc." v-model="form.suffix"
            @input="validateSuffix">
          <label for="suffix">Suffix: <span class="optional">(Optional)</span></label>
        </div>
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('birthdate')">{{ getWarning('birthdate') }}</span>
    ...
    <div class="form-content" v-if="step === 'login_details'">
      ...
      <div class="registration-box">
        ...
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('barangay')">{{ getWarning('barangay') }}</span>
        ...
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('province')">{{ getWarning('province') }}</span>
          <input type="text" id="province" name="province" v-model="form.province" required @input="validateProvince">
          <label for="province">Province: <span>*</span></label>
        </div>
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('country')">{{ getWarning('country') }}</span>
        ...
        <div class="form-group">
          ...
          <div class="password-input-wrapper">
            ...
            <svg v-else @click="showPassword = !showPassword" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
        ...
        <div class="form-group">
          <span class="field-warning" v-if="getWarning('repassword')">{{ getWarning('repassword') }}</span>
          <div class="password-input-wrapper">
            ...
            <!-- Show/Hide Password Icon -->
            <svg v-if="!showRePassword" @click="showRePassword = !showRePassword" xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24" fill="currentColor" class="size-6 eye-icon">
              <path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
              <path fill-rule="evenodd"
                d="M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 0 1 0-1.113ZM17.25 12a5.25 5.25 0 1 1-10.5 0 5.25 5.25 0 0 1 10.5 0Z"
            ...
            <svg v-else @click="showRePassword = !showRePassword" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
              fill="currentColor" class="size-6 eye-icon">
              <path
                d="M3.53 2.47a.75.75 0 0 0-1.06 1.06l18 18a.75.75 0 1 0 1.06-1.06l-18-18ZM22.676 12.553a11.249 11.249 0 0 1-2.631 4.31l-3.099-3.099a5.25 5.25 0 0 0-6.71-6.71L7.759 4.577a11.217 11.217 0 0 1 4.242-.827c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113Z" />
              <path
    ...
    <div class="form-content" v-if="step === 'questions'">
      ...
      <hr>
      <div class="registration-box" style="display: grid; grid-template-columns: repeat(2, 1fr);">
        <div class="form-group">
          <select id="question1" v-model="form.question1" required>
            <option disabled value="">-- Select a question --</option>
            <option v-for="(q, index) in questionList" :key="'q1-' + index" :value="q.choice"
        ...
        <div class="form-group">
          ...
          <div class="password-input-wrapper answer-input-wrapper">
            <input :type="showAnswer1 ? 'text' : 'password'" id="answer1" v-model="form.answer1" required
              @input="validateAnswer" autocomplete="off">
        ...
        <div class="form-group">
          ...
          <div class="password-input-wrapper answer-input-wrapper">
            <input :type="showAnswer2 ? 'text' : 'password'" id="answer2" v-model="form.answer2" required
              @input="validateAnswer" autocomplete="off">
            <button type="button" class="answer-toggle eye-icon"
        ...
        <div class="form-group">
          ...
          <div class="password-input-wrapper answer-input-wrapper">
            <input :type="showAnswer3 ? 'text' : 'password'" id="answer3" v-model="form.answer3" required
              @input="validateAnswer" autocomplete="off">
            <button type="button" class="answer-toggle eye-icon"
